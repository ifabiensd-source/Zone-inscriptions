...
const formatDate = (startDateString: string, endDateString?: string) => {
  ...
  if (!endDateString || startDateString === endDateString) {
    return new Date(startDateString).toLocaleDateString('fr-FR', options);
  }

  const endDate = new Date(endDateString);

  if (startDate.getFullYear() !== endDate.getFullYear()) {
    return `Du ${startDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' })} au ${endDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' })}`;
  }
  
  if (startDate.getMonth() !== endDate.getMonth()) {
    return `Du ${startDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', timeZone: 'UTC' })} au ${endDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' })}`;
  }

  return `Du ${startDate.getUTCDate()} au ${endDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' })}`;
};


const ActivityCard: React.FC<ActivityCardProps> = ({ activity, onRegister, isAdmin, onDelete, onEdit, loggedInUser }) => {
  const { id, title, description, date, endDate, startTime, endTime, spots, registrations, serviceAllocations, ageRestriction } = activity;
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex justify-between items-start gap-4 mb-2">
            ...
            <div className="text-right flex-shrink-0">
                <p className="font-semibold text-sm text-[var(--color-primary-accent)] capitalize">{formatDate(date, endDate)}</p>
            ...
            <div className="pl-2 border-l-2 border-border">
               <div className={`space-y-1 ${isExpanded ? 'max-h-32 overflow-y-auto custom-scrollbar pr-2' : ''}`}>
                 {participantsToShow.map(reg => (
                    <p key={reg.id} className="text-text truncate" title={`${getYouthName(reg)} (${reg.department})`}>
                      {getYouthName(reg)} <span className="text-text-muted text-xs italic">({reg.department})</span>
                    </p>
                  ))}
               </div>
