...
const SchedulePDFTemplate: React.FC<SchedulePDFTemplateProps> = ({ service, activities, theme }) => {
    ...
    const activitiesByDate = useMemo(() => {
        ...
        activities.forEach(activity => {
            ...
            const dateActivities = groups.get(date)!;
            // Insert sorted by start time
            const insertIndex = dateActivities.findIndex(a => timeToMinutes(a.startTime) > timeToMinutes(activity.startTime));
            if (insertIndex === -1) {
                dateActivities.push(activity);
            } else {
                dateActivities.splice(insertIndex, 0, activity);
            }
        });
                        ...
                        <div key={activity.id} style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ flexGrow: 1, borderLeft: `3px solid ${brandColor}`, paddingLeft: '20px' }}>
                            <div style={{ 
                              ...
                              <h3 style={{ fontSize: '18px', fontWeight: 600, color: textColor, margin: 0, wordBreak: 'break-word' }}>{activity.title}</h3>
